...
var angle1 = zcell1.GetSheet(0).GetCellValue(11,i);
	...
	for (var i = 1; i <datastr.length; i++) {
		...
		zcell1.GetSheet(0).SetCellReadOnly(13, i, 1);
...
function checkAgree() {
	...
	var url = path + "/Tower/checkAgreeTower.action";
	 $.ajax({
		 type:'post',
		 url:url,
		 data:{
...
function checkReject() {
	params = JSON.stringify(param);
	var data = getParams();
	var url = path + "/Tower/checkTower.action";
	 $.ajax({
		 type:'post',
		 url:url,
		 data:{
...
function getConnectParams(){
	...
	for(var i=1;i<datastr.length;i++){
		var obj = new Object;
		var str11 = datastr[i][11];
		if(str11=='不得接头'&&datastr[i-1][11]=='不得接头'&&datastr[i][0]!=datastr[i-1][0]){
			obj=datastr[i][0]+datastr[i][1];
			list.push(obj);
		}	
}
...
function getCountParams(){
	...
	for(var i=1;i<datastr.length;i++){
		...
		list0.push(datastr[i][0])
		list1.push(datastr[i][1])
		list2.push(datastr[i][2])
		list3.push(datastr[i][3])
		list4.push(datastr[i][4])
		list5.push(datastr[i][5])
		list6.push(datastr[i][6])
		list7.push(datastr[i][7])
		list8.push(datastr[i][8])
		list9.push(datastr[i][9])
		list10.push(datastr[i][10])
		list11.push(datastr[i][11])
		list12.push(datastr[i][12])
		list13.push(datastr[i][13])
		
	}
	list = [list0,list1,list2,list3,list4,list5,list6,list7,list8,list9,list10,list11,list12,list13]
	data = JSON.stringify(list);
	return data;
