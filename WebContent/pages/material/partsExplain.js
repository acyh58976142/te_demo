...
partsExplain.prototype.JSONToExcelConvertor = function(){
	...
	exportExcel("materialTable",timestamp+"组配件明细表说明");
}
	...
	$.ajax({
		...
		success : function(data) {
			if (!Tools.isEmpty(data)) {
				$("#pageCode").text(projectCode+"-D08");//工程编号
...
partsExplain.prototype.initHammerTable = function(){
	...
	for(var i = 0; i <wireHammer.length; i++){
		tr+="<tr><td>"+sortNo+"</td><td>防振锤</td><td>"+hammerTypeArr[wireHammer[i][0]]+"</td><td>(只)</td><td>"
		+wireHammer[i][1]+"</td><td></td></tr>"
		sortNo++;
	}
	$("#materialTr").before(tr);
	...
	for(var i = 0; i <groundHammer.length; i++){
		tr+="<tr><td>"+sortNo+"</td><td>防振锤</td><td>"+hammerTypeArr[groundHammer[i][0]]+"</td><td>(只)</td><td>"
		+groundHammer[i][1]+"</td><td></td></tr>"
		sortNo++;
	}
	$("#materialTr").before(tr);
	...
	for(var i = 0; i <OPGWHammer.length; i++){
		tr+="<tr><td>"+sortNo+"</td><td>防振锤</td><td>"+hammerTypeArr[OPGWHammer[i][0]]+"</td><td>(只)</td><td>"
		+OPGWHammer[i][1]+"</td><td></td></tr>"
		sortNo++;
	}
	$("#materialTr").before(tr);
...
partsExplain.prototype.createWireTable = function(){
	...
	for (var i = 0; i <countlists.length; i++) {
		tr+="<tr><td>"+sortNo+"</td><td>导,地线,OPGW光缆金具串</td><td>"+countlists[i][0]+"</td><td>(串)</td><td>"
		+countlists[i][1]+"</td><td></td></tr>"
		sortNo++;
	}
	$("#materialTr").before(tr);
	}
...
partsExplain.prototype.createOPGWTable = function(){
	...
	for (var i = 0; i <OPGWlists.length; i++) {
		tr+="<tr><td>"+sortNo+"</td><td>OPGW光缆分盘</td><td>"+OPGWlists[i][0]+"</td><td>(m)</td><td>"
		+OPGWlists[i][1]+"</td><td></td></tr>"
		sortNo++;
	}
	tr+="<tr><td>"+sortNo+"</td><td>OPGW光缆分盘</td><td>合计</td><td>(m)</td><td>"
	+count+"</td><td></td></tr>"
	sortNo++;
	$("#materialTr").before(tr);
	
	partsExplain.queryMainInfo();
	}

/**
 * 导线重量数据查询Ajax
 **/
partsExplain.prototype.queryMainInfo = function() {
	var url = path+"/material/getPartsDataI.action";
	
	$.ajax({
		"type": "post",	//post防止中文参数乱码
		"url": url,
		"data":{projectId:id,projectName:projectName},
		"dataType": "json",  
		"success": function(data){
			var tr = "";
			for(var i=0;i<data.length;i++){
				var groundWireWeight = data[i];
					tr+= "<tr><td>"+sortNo+"</td><td>导地线重量</td>";
					tr+= "<td>"+groundWireWeight[0]+"</td><td></td>";
					tr+= "<td>"+parseFloat(groundWireWeight[1]).toFixed(2)+"</td><td></td></tr>";		
					sortNo++;
			}
			$("#materialTr").before(tr);	
